<script lang="ts">
  ...
  import { buildCoverageEnvelope, buildEmpireInfluence } from '../territory';
  ...
  $: influenceResult = buildEmpireInfluence(systems, { width: 1000, height: 620, step: 12, minimumControl: 0.13, contestedRatio: 0.76 });
  $: empireTerritories = influenceResult.territories.map((territory) => ({
    ownerPlayerId: territory.empireId,
    owner: ownerForPlayerId(territory.empireId, playerIds),
    memory: territory.memory,
    mixedIntel: territory.mixedIntel,
  ...
  <div class="minimap" aria-hidden="true">
    <svg viewBox="0 0 100 70">
      ...
      {#if liveMode}
        ...
        {#each empireTerritories as territory}
          {#each territory.paths as path}
            <path class="mini-territory-border" class:memory={territory.memory} style={`--mini-color:${OWNER_COLORS[territory.owner]}`} d={path} transform="scale(.1)"/>
          {/each}
        {/each}
      {:else}
