...
<aside class="detail-panel" class:stale={isStale} style={`--system-owner-color:${systemColor}`}>
  ...
  <div class="summary-grid">
    ...
    <div><Icon name="industry" size={19}/><span><small>Development</small><strong>{system.development}%</strong></span></div>
  ...
  <section class="panel-section">
    ...
    <div class="installed-models">
      {#each (system.installations ?? []) as installation}
        ...
        {@const queued = draftUpgrade(installation.family)}
        <span><strong>{installation.name}</strong><small>v{installation.version} · active{upgrade ? ` · upgrading to ${upgrade.toName} (${upgrade.turnsRemaining} turn${upgrade.turnsRemaining === 1 ? '' : 's'})` : queued ? ` · ${(queued.modelName ?? queued.item)} in queue` : ''}</small></span>
      {/each}
    ...
    <div class="build-options">
      ...
      {#each installationFamilies as family}
        ...
        {:else if draft}
          <button disabled class="queued-action"><Icon name={family === 'defense_grid' ? 'shield' : family === 'deep_space_array' ? 'target' : 'industry'} size={15}/><span><strong>{draft.modelName ?? draft.item}</strong><small>IN QUEUE · starts when this turn is processed</small></span></button>
...
<style>
  ...
  .summary-grid{display:grid;grid-template-columns:repeat(2,1fr);border-bottom:1px solid rgba(62,164,218,.2)}.summary-grid>div{min-height:54px;display:flex;align-items:center;gap:.6rem;padding:0 .8rem;border-right:1px solid rgba(62,164,218,.16);border-bottom:1px solid rgba(62,164,218,.12);color:#75cfff}.summary-grid>div:nth-child(even){border-right:0}.summary-grid>div:nth-last-child(-n+2){border-bottom:0}.summary-grid small,.summary-grid strong{display:block}.summary-grid small{color:#7991a2;font-size:.65rem}.summary-grid strong{color:#dcebf4;font-size:.8rem;margin-top:.1rem}.happy{color:#8cdd60;font-size:1.45rem}
  ...
  .installed-models{display:flex;gap:.3rem;flex-wrap:wrap;margin:.55rem 0}.installed-models span{padding:.28rem .36rem;border:1px solid rgba(59,145,188,.18);background:rgba(5,24,38,.55)}.installed-models strong,.installed-models small{display:block}.installed-models strong{color:#afc6d2;font-size:.54rem;font-weight:500}.installed-models small{margin-top:.08rem;color:#677f90;font-size:.45rem}.build-options{display:grid;gap:.38rem;margin-top:.65rem}.build-options button{min-height:42px;display:grid;grid-template-columns:20px 1fr;gap:.45rem;align-items:center;padding:.4rem .55rem;border:1px solid rgba(61,160,209,.25);background:rgba(5,27,42,.72);color:#58caff;text-align:left;cursor:pointer}.build-options button:hover:not(:disabled){border-color:#48caff;background:rgba(10,49,72,.86)}.build-options button:disabled{opacity:.35;cursor:not-allowed}.build-options button.work-action{border-color:rgba(217,181,72,.25);background:rgba(48,37,8,.25);opacity:.72}.build-options button.queued-action{border-color:rgba(126,145,156,.22);background:rgba(24,31,36,.58);color:#8297a2;opacity:.58;filter:saturate(.35)}.build-options button.queued-action strong{color:#9eafb8}.build-options button.queued-action small{color:#778892}.build-options button.current-action{opacity:.55}.build-options button.locked{opacity:.42}.build-options span,.build-options strong,.build-options small{display:block}.build-options strong{color:#c7dbe6;font-size:.67rem;font-weight:500}.build-options small{margin-top:.12rem;color:#69899d;font-size:.56rem}.build-hint{margin:.5rem 0 0;color:#71899b;font-size:.62rem;line-height:1.4}
