<script lang="ts">
  ...
  const installationFamilies = ['mining_complex', 'power_plant', 'hydroponics', 'research_complex', 'asteroid_mining', 'defense_grid', 'orbital_factory', 'deep_space_array'] as const;
  const resourceRows: { id: 'industry' | 'energy' | 'bio' | 'science'; label: string; icon: string }[] = [
    { id: 'industry', label: 'Industry', icon: 'industry' },
    { id: 'energy', label: 'Energy', icon: 'energy' },
    { id: 'bio', label: 'Biomass', icon: 'planet' },
    { id: 'science', label: 'Science', icon: 'research' }
  ];
  ...
  function createBuildOptions(): BuildOption[] {
    ...
    for (const family of installationFamilies) {
      ...
      const detail = family === 'defense_grid'
        ...
        : family === 'orbital_factory'
          ? `+${stats.industryIncome ?? 0} industrial processing/turn`
          : family === 'deep_space_array'
            ? `sensor range ${stats.sensorRange ?? 1}`
            : family === 'asteroid_mining'
              ? `asteroid extraction rating ${stats.asteroidYield ?? 0}`
              : `${stats.resourceYield ?? 0} extraction rating`;
      result.push({ item: model.name, modelId: model.id, cost: stats.industryCost ?? 0, icon: installationIcon(family), detail, family, kind: 'installation' });
    }
    return result;
  }

  function groupProductionBySystem(production: ProductionOrder[]): Map<string, ProductionOrder[]> {
    const grouped = new Map<string, ProductionOrder[]>();
    for (const order of production) {
      const current = grouped.get(order.systemId);
      if (current) current.push(order);
      else grouped.set(order.systemId, [order]);
    }
    return grouped;
  }

  function queueFor(systemId: string): ProductionOrder[] {
    return productionBySystem.get(systemId) ?? [];
  }
  ...
  function statusLabel(system: StarSystem): string {
    ...
    const queuedUpgrades = queue.filter((order) => order.productionKind === 'upgrade').length;
    const queuedBuilds = queue
      .filter((order) => order.productionKind !== 'upgrade')
      .reduce((sum, order) => sum + order.quantity, 0);
    if (upgrades.length > 0) return upgrades.length === 1 ? 'UPGRADING' : `UPGRADING ×${upgrades.length}`;
    if (queuedUpgrades > 0 && queuedBuilds > 0) return 'ORDERS QUEUED';
    if (queuedUpgrades > 0) return queuedUpgrades === 1 ? 'UPGRADE QUEUED' : `UPGRADES QUEUED ×${queuedUpgrades}`;
    if (queuedBuilds > 0) return `BUILDING ×${queuedBuilds}`;
    return 'IDLE';
  }
  ...
  function potential(system: StarSystem, resourceId: 'industry' | 'energy' | 'bio' | 'science'): number | null {
    const value = system.resourcePotential?.[resourceId];
    return typeof value === 'number' ? value : null;
  }

  function outputPart(system: StarSystem, resourceId: string, part: 'base' | 'planetary' | 'asteroid' | 'orbital' | 'total'): number {
    return system.resourceOutput?.[resourceId]?.[part] ?? (part === 'total' ? (resource(system, resourceId)?.income ?? 0) : 0);
  }

  function extractionFamily(resourceId: 'industry' | 'energy' | 'bio' | 'science'): string {
    return resourceId === 'industry' ? 'mining_complex' : resourceId === 'energy' ? 'power_plant' : resourceId === 'bio' ? 'hydroponics' : 'research_complex';
  }

  function extractionName(system: StarSystem, resourceId: 'industry' | 'energy' | 'bio' | 'science'): string {
    return installedForFamily(system, extractionFamily(resourceId))?.name ?? 'No extraction installation';
  }
  ...
  function canQueue(system: StarSystem, option: BuildOption): boolean {
    ...
    if (option.kind === 'installation' && option.family) {
      if (option.family === 'asteroid_mining' && system.asteroids?.present !== true) return false;
  ...
  function installationIcon(family: string): string {
    ...
    if (family === 'power_plant') return 'energy';
    if (family === 'hydroponics') return 'planet';
    if (family === 'research_complex') return 'research';
    if (family === 'asteroid_mining') return 'fleet';
    return 'industry';
  }

  function installationBuildTitle(system: StarSystem, family: string, cost: number): string {
    if (!editableTurn) return 'Reopen the turn to change production';
    if (family === 'asteroid_mining' && system.asteroids?.present !== true) return 'No significant asteroid deposit in this system';
    if (remainingIndustry(system) < cost) return `${cost.toLocaleString('en-US')} industry required`;
    return `${cost.toLocaleString('en-US')} industry`;
  }
...
<section class="planets-view">
  <header class="view-header">
    <div>
      ...
      <h1>Planets</h1>
      <p class="intro">Colonies start collapsed for a compact empire overview. Natural potential determines what a world is good at; extraction installations turn that potential into real resources. Expand a colony to manage economy, upgrades and production.</p>
    </div>
  ...
  {#if visibleColonies.length > 0}
    <div class="planet-list">
      {#each visibleColonies as system}
        ...
        <details class="planet-card panel-cut" class:building={isBusy(system)} class:idle={!isBusy(system)}>
          <summary class="planet-summary-row" title={`Expand or collapse ${system.name}`}>
            <span class="expand-icon"><Icon name="chevron-right" size={16}/></span>
            <span class="planet-identity">
              <span class="planet-icon"><Icon name="planet" size={22}/></span>
          ...
          <div class="expanded-content">
              ...
              <section class="resource-block">
                <div class="section-title"><span>Resource extraction</span><small>Natural potential × installed infrastructure determines recurring output.</small></div>
                <div class="resource-grid">
                  {#each resourceRows as entry}
                    {@const resourcePotential = potential(system, entry.id)}
                    <div class="resource-card">
                      <span class="resource-card-icon"><Icon name={entry.icon} size={17}/></span>
                      <span class="resource-card-main"><strong>{entry.label}</strong><small>{extractionName(system, entry.id)}</small></span>
                      <span class="resource-card-stat"><small>Potential</small><strong>{resourcePotential === null ? '—' : `${resourcePotential}%`}</strong></span>
                      <span class="resource-card-stat"><small>Output</small><strong>+{outputPart(system, entry.id, 'total')}</strong></span>
                      <span class="resource-card-detail">base {outputPart(system, entry.id, 'base')} · planet {outputPart(system, entry.id, 'planetary')}{entry.id === 'industry' ? ` · asteroid ${outputPart(system, entry.id, 'asteroid')} · orbital ${outputPart(system, entry.id, 'orbital')}` : ''}</span>
                    </div>
                  {/each}
                </div>
                <div class="asteroid-row" class:available={system.asteroids?.present === true}>
                  <span><Icon name="fleet" size={17}/><strong>{system.asteroids?.label ?? 'Asteroid survey pending'}</strong></span>
                  <span><small>Richness</small><strong>{system.asteroids?.present === true ? `${system.asteroids.richness}%` : '—'}</strong></span>
                  <span><small>Mining</small><strong>{installedForFamily(system, 'asteroid_mining')?.name ?? (system.asteroids?.present === true ? 'Station not built' : 'Unavailable')}</strong></span>
                  <span><small>Industry</small><strong>+{outputPart(system, 'industry', 'asteroid')}</strong></span>
                </div>
              </section>
                  ...
                  {#if (system.installations ?? []).length > 0}
                    {#each (system.installations ?? []) as installation}
                      ...
                      <div class="installation-entry" class:upgrade-active={Boolean(pending || draft)}>
                        <span class="installation-model"><Icon name={installationIcon(installation.family)} size={15}/><span><strong>{installation.name}</strong><small>v{installation.version} · active{draft ? ` · ${draft.modelName ?? draft.item} in queue` : ''}</small></span></span>
                        {#if pending}
                        ...
                        {:else if draft}
                          <span class="upgrade-progress queued"><strong>IN QUEUE · {draft.modelName ?? draft.item}</strong><small>Upgrade starts when this turn is processed · {draft.upgradeTurns ?? 2} turns total</small></span>
                        {:else if target}
                  ...
                  <div class="queue-list">
                    {#each queue as order}
                      <div class="queue-item">
                        <span class="queue-icon"><Icon name={order.modelId?.startsWith('scout-') || order.item === 'Scout Wing' ? 'fleet' : installationIcon(modelCatalog?.installations.find((model) => model.id === order.modelId)?.family ?? '')} size={16}/></span>
              ...
              <div class="build-actions">
                ...
                {#each installationFamilies as family}
                  ...
                  {:else if draft}
                    <button disabled class="queued-action"><Icon name={installationIcon(family)} size={16}/><span><strong>{draft.modelName ?? draft.item}</strong><small>IN QUEUE · starts when this turn is processed</small></span></button>
                  ...
                  {:else if buildModel}
                    ...
                    <button disabled={!buildOption || !canQueue(system, buildOption)} onclick={() => onQueueBuild(system, buildModel.name, buildModel.id)} title={installationBuildTitle(system, family, buildModel.stats.industryCost ?? 0)}>
                      <Icon name={installationIcon(family)} size={16}/>
...
<style>
  ...
  .resource-block{padding:.7rem .8rem;border-bottom:1px solid rgba(57,132,173,.14);background:rgba(3,17,28,.42)}.resource-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:.4rem}.resource-card{display:grid;grid-template-columns:30px minmax(0,1fr) 68px 62px;gap:.45rem;align-items:center;padding:.45rem .5rem;border:1px solid rgba(63,151,194,.18);background:rgba(5,25,39,.55)}.resource-card-icon{width:28px;height:28px;display:grid;place-items:center;color:#5cccf7;border:1px solid rgba(70,185,232,.2)}.resource-card-main strong,.resource-card-main small,.resource-card-stat strong,.resource-card-stat small{display:block}.resource-card-main strong{color:#d2e3eb;font-size:.62rem;font-weight:500}.resource-card-main small{margin-top:.1rem;color:#6d899a;font-size:.48rem}.resource-card-stat{border-left:1px solid rgba(58,132,171,.14);padding-left:.45rem}.resource-card-stat small{color:#617d90;font-size:.46rem;text-transform:uppercase;letter-spacing:.05em}.resource-card-stat strong{margin-top:.08rem;color:#91d9f5;font-size:.65rem}.resource-card-detail{grid-column:2/-1;color:#5d788a;font-size:.46rem;line-height:1.3}.asteroid-row{display:grid;grid-template-columns:minmax(0,1.5fr) 80px minmax(140px,1fr) 70px;gap:.55rem;align-items:center;margin-top:.4rem;padding:.45rem .55rem;border:1px solid rgba(112,124,135,.18);background:rgba(11,17,22,.55);opacity:.65}.asteroid-row.available{border-color:rgba(212,171,66,.28);background:rgba(45,34,8,.2);opacity:1}.asteroid-row>span:first-child{display:flex;align-items:center;gap:.4rem;color:#d3ad50}.asteroid-row strong,.asteroid-row small{display:block}.asteroid-row strong{color:#cbdde5;font-size:.56rem;font-weight:500}.asteroid-row small{color:#667f90;font-size:.45rem;text-transform:uppercase;letter-spacing:.05em}
