  ...
  {#if visibleColonies.length > 0}
    <div class="planet-list">
      {#each visibleColonies as system}
        ...
        <article class="planet-card panel-cut" class:idle={queue.length === 0}>
          <div class="planet-main">
            <button class="planet-title" onclick={() => onLocate(system)}>
              <span class="planet-icon"><Icon name="planet" size={24}/></span>
              ...
              <div class="queue-list">
                {#each queue as order}
                  <div class="queue-item">
                    <span class="queue-icon"><Icon name={order.item === 'Scout Wing' ? 'fleet' : order.item === 'Defense Grid' ? 'shield' : 'industry'} size={16}/></span>
