...
<section class="planets-view">
  <header class="view-header">
    <div>
      ...
      <h1>Planets</h1>
      <p class="intro">Manage every colony from one screen. New installations complete this turn; upgrades take multiple turns and keep the old model active until completion.</p>
    </div>
  ...
  <div class="toolbar panel-cut">
    ...
    <p>Projected industry includes this turn's income. Build and upgrade costs are reserved when queued.</p>
  </div>
  ...
  {#if visibleColonies.length > 0}
    <div class="planet-list">
      {#each visibleColonies as system}
        ...
        <article class="planet-card panel-cut" class:building={isBusy(system)} class:idle={!isBusy(system)}>
          <div class="planet-main">
            ...
            <div class="status" class:idle-status={!isBusy(system)}>
              {#if upgrades.length > 0}<strong>UPGRADING</strong><small>{upgrades.length} in progress</small>
              {:else if queue.length > 0}<strong>BUILDING</strong><small>{queue.reduce((sum, order) => sum + order.quantity, 0)} queued</small>
              {:else}<strong>IDLE</strong><small>No order this turn</small>{/if}
            </div>
          </div>
          ...
          <section class="installations-block">
            <div class="section-title"><span>Installed models</span><small>Hardware stays on its current version until an explicit upgrade completes.</small></div>
            <div class="installation-list">
              {#if (system.installations ?? []).length > 0}
                ...
                {/each}
              {:else}
                <div class="installation-entry none"><span class="installation-model"><Icon name="build" size={14}/><span><strong>No registered installations</strong><small>legacy/base colony</small></span></span></div>
              {/if}
            </div>
          </section>

          <section class="queue-block">
            <div class="section-title"><span>Orders this turn</span><button onclick={() => onLocate(system)}>Open planet <Icon name="galaxy" size={14}/></button></div>
            {#if queue.length > 0}
            ...
            {:else}
              <div class="idle-message"><Icon name="build" size={20}/><span><strong>This system has no orders this turn.</strong><small>Choose a build below or upgrade an installed model above.</small></span></div>
            {/if}
          </section>

          <div class="build-actions">
            {#each buildOptions as option}
              <button disabled={!canQueue(system, option)} onclick={() => onQueueBuild(system, option.item, option.modelId)} title={!editableTurn ? 'Reopen the turn to change production' : option.family && installedFamily(system, option.family) ? 'This installation family is already present; use its explicit upgrade action above' : `${option.cost} industry`}>
...
<style>
  ...
  @media(max-width:1200px){.installation-list{grid-template-columns:1fr}.view-header{display:grid}.planet-summary{justify-content:flex-start}.planet-summary span{text-align:left}.metrics{grid-template-columns:repeat(3,1fr)}.metrics>div:nth-child(3){border-right:0}.metrics>div:nth-child(-n+3){border-bottom:1px solid rgba(57,132,173,.12)}.build-actions{grid-template-columns:1fr}.toolbar{display:grid}.toolbar p{text-align:left}}
  @media(max-width:720px){.planets-view{padding:.8rem}.planet-main{align-items:flex-start}.metrics{grid-template-columns:repeat(2,1fr)}.metrics>div{border-bottom:1px solid rgba(57,132,173,.12)}.metrics>div:nth-child(odd){border-right:1px solid rgba(57,132,173,.12)}.metrics>div:nth-child(even){border-right:0}.planet-summary{gap:.8rem}.filters{overflow-x:auto}.toolbar p{display:none}.queue-item{grid-template-columns:28px minmax(0,1fr) 34px 34px}.installation-entry{grid-template-columns:1fr}.upgrade-progress{min-width:0}}
</style>
