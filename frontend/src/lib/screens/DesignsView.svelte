<script lang="ts">
  ...
  const requiredComponentCategories: ShipComponentCategory[] = ['hull', 'engine', 'scanner', 'weapon', 'armor'];
  const componentCategories: ShipComponentCategory[] = [...requiredComponentCategories, 'utility'];
  ...
  $: preview = editorBase ? designPreview(selectedComponents) : null;
  ...
  function componentIcon(category: string): string {
    ...
    if (category === 'utility') return 'colonize';
  ...
  function startClone(design: ShipDesign): void {
    ...
    selectedComponents = {
      ...
      utility: componentId(design, 'utility')
    };
    const colonyDesign = (design.stats.colonizationCapacity ?? 0) > 0;
    designName = `${colonyDesign ? 'Colony Ship' : `${design.family.charAt(0).toUpperCase()}${design.family.slice(1)}`} Mk ${roman(nextGeneration)}`;
  }
  ...
  function chooseComponent(category: ShipComponentCategory, event: Event): void {
    ...
    const previousUtility = selectedComponents.utility;
    selectedComponents = { ...selectedComponents, [category]: select.value };
    if (category === 'utility' && previousUtility !== select.value) {
      if (select.value === 'colony_module_mk1' && /^Scout Mk /.test(designName)) designName = `Colony Ship Mk ${roman(nextGeneration)}`;
      if (select.value === '' && /^Colony Ship Mk /.test(designName)) designName = `Scout Mk ${roman(nextGeneration)}`;
    }
  }
  ...
  function useNewestUnlocked(): void {
    ...
    for (const category of requiredComponentCategories) {
  ...
  function designPreview(selection: Record<ShipComponentCategory, string>) {
    ...
    if (selection.utility) {
      const utility = components.find((candidate) => candidate.id === selection.utility && candidate.category === 'utility' && candidate.unlocked);
      if (!utility) return null;
      selected.set('utility', utility);
    }
    ...
    const utility = selected.get('utility');
    const colonizationCapacity = Math.max(0, utility?.stats.colonizationCapacity ?? 0);
    const industryCost = [...selected.values()].reduce((sum, model) => sum + (model.stats.industryCost ?? 0), 0);
    ...
    return {
      ...
      colonizationCapacity,
      industryCost: Math.max(300, industryCost),
  ...
  function pendingComponentName(order: ShipDesignOrder, category: ShipComponentCategory): string {
    ...
    if (category === 'utility' && !modelId) return 'None';
...
<section class="designs-view">
  <header class="view-header">
    <div>
      ...
      <p class="intro">Research unlocks hardware models, but it never changes a ship already in service and no longer creates a new design automatically. Clone an existing design, choose researched components and queue a new immutable generation. The optional utility slot can carry a Colony Module Mk I for repeatable expansion.</p>
    </div>
    <div class="summary">
  ...
  {#if mode === 'ships'}
    {#if currentDesign}
      <section class="current-design panel-cut">
        ...
        <div class="stats">
          ...
          <span><small>Colony</small><strong>{currentDesign.stats.colonizationCapacity ?? 0}</strong></span>
      ...
      <section class="design-editor panel-cut">
          ...
          {#each componentCategories as category}
            ...
            <label>
              ...
              <select value={selectedComponents[category]} onchange={(event) => chooseComponent(category, event)}>
                {#if category === 'utility'}<option value="">No utility module</option>{/if}
                {#each componentOptions(category) as model}
                  <option value={model.id}>{model.name} · v{model.version}</option>
                {/each}
              </select>
              <small>{selected?.description ?? (category === 'utility' ? 'Optional. Colony Module Mk I makes this a single-use colony ship.' : 'No unlocked model.')}</small>
            </label>
          {/each}
        </div>
        ...
        {#if preview}
          <div class="preview">
            ...
            <span><small>Colony</small><strong>{preview.colonizationCapacity}</strong></span>
    ...
    <div class="design-history">
      {#each designs as design}
        <article class="design-card panel-cut" class:current={design.current} class:obsolete={design.obsolete}>
          ...
          <div class="mini-stats"><span>SPD {design.stats.movementRange}</span><span>SEN {design.stats.sensorRange}</span><span>ATK {design.stats.attack}</span><span>DEF {design.stats.defense}</span>{#if (design.stats.colonizationCapacity ?? 0) > 0}<span>COL {design.stats.colonizationCapacity}</span>{/if}</div>
    ...
    <div class="future panel-cut"><Icon name="build" size={19}/><span><strong>Design generations are now player-created.</strong><small>Colony ships are now ordinary immutable designs too. Planet production can build any saved generation exactly; fleet refit remains the next larger 0.7.2 step.</small></span></div>
