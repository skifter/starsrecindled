<script lang="ts">
  ...
  function addProduction(item: string): void {
    ...
    if (!demoMode) {
      localNotice = 'Production processing is not enabled yet. Fleet movement is the first live gameplay action.';
      return;
    }
  ...
  <div class="game-grid" class:panel-closed={!rightPanelOpen || activeSection === 'players'}>
    ...
    <main class="content-area">
      ...
      {:else if activeSection === 'galaxy'}
        <GalaxyMap systems={gameSystems} routes={gameRoutes} {plannedRoutes} selectedId={selectedSystem?.id ?? ''} liveMode={!demoMode} onSelect={selectSystem}/>
      {:else if !demoMode}
