<script lang="ts">
  ...
  function currentFuel(fleet: FleetSummary): number {
    return Math.max(0, Number(fleet.fuel ?? fleet.fuelCapacity ?? 0));
  }

  function fuelCapacity(fleet: FleetSummary): number {
    return Math.max(0, Number(fleet.fuelCapacity ?? 0));
  }

  function fuelRange(fleet: FleetSummary): number {
    if (typeof fleet.operationalRange === 'number') return Math.max(0, fleet.operationalRange);
    const use = Math.max(0, Number(fleet.fuelUsePerHop ?? 0));
    return use > 0 ? Math.floor(currentFuel(fleet) / use) : 0;
  }
...
<section class="fleets-view">
  <header class="view-header">
    <div>
      ...
      <h1>Fleets</h1>
      <p class="intro">Fleets preserve exact ship generations, persistent fuel and explicit refit state. Movement now consumes fuel per hop; fleets that begin a processing cycle at one of your colonies automatically refuel before their orders are resolved.</p>
    </div>
      ...
      {#each ownRows as row}
        ...
        <article class="fleet-row own" class:selected={row.fleet.id === selectedFleetId || row.fleet.id === managedFleetId} style={`--fleet-color:${fleetColor(row.fleet)}`}>
          ...
          <span class="order-state">
            ...
            {/if}
            <small class="capability">SPD {row.fleet.movementRange ?? 1} · FUEL {currentFuel(row.fleet)}/{fuelCapacity(row.fleet)} · RNG {fuelRange(row.fleet)} · SEN {row.fleet.sensorRange ?? 0} · ATK {row.fleet.attack ?? 0} · DEF {row.fleet.defense ?? 0}{#if (row.fleet.colonizationCapacity ?? 0) > 0} · COL {row.fleet.colonizationCapacity}{/if}</small>
          </span>
          <span class="actions">
  ...
  {#if managedRow}
    ...
    <section class="fleet-manager panel-cut">
      ...
      <div class="fuel-banner">
        ...
        <span>
          <strong>Fuel {currentFuel(managedRow.fleet)} / {fuelCapacity(managedRow.fleet)} · operational range {fuelRange(managedRow.fleet)} hop(s)</strong>
          <small>{managedRow.fleet.fuelUsePerHop ?? 0} fuel per hop after research modifiers.{#if managedRow.system.ownerPlayerId === currentPlayerId} This fleet will refuel automatically to full before movement is resolved.{:else} Reach one of your colonies to refuel on a later processing cycle.{/if}</small>
        </span>
      </div>
...
<style>
  ...
  .fleet-manager{margin-top:1rem;border:1px solid rgba(75,177,219,.3);background:linear-gradient(180deg,rgba(5,25,40,.97),rgba(3,13,23,.97));padding:1rem}.fleet-manager>header{display:flex;justify-content:space-between;gap:1rem;align-items:flex-start;border-bottom:1px solid rgba(63,143,181,.18);padding-bottom:.8rem}.fleet-manager h2{margin:0;color:#e6f5fc;font-size:1.05rem;font-weight:500}.fleet-manager header p:last-child{margin:.35rem 0 0;color:#6e8a9c;font-size:.64rem}.close-manager,.draft-banner button{border:1px solid rgba(72,159,201,.25);background:rgba(5,25,40,.8);color:#76cdef;font:inherit;font-size:.58rem;padding:.45rem .65rem;cursor:pointer}.draft-banner,.refit-banner,.fuel-banner{margin-top:.75rem;display:flex;align-items:center;justify-content:space-between;gap:.7rem;padding:.65rem .75rem;border:1px solid rgba(255,208,92,.26);background:rgba(78,57,9,.15);color:#d9be6d;font-size:.65rem}.refit-banner,.fuel-banner{justify-content:flex-start}.refit-banner{border-color:rgba(91,210,163,.3);background:rgba(8,65,47,.16);color:#6ed9aa}.fuel-banner{border-color:rgba(83,177,232,.3);background:rgba(8,46,72,.2);color:#67c9f4}.refit-banner span,.fuel-banner span{min-width:0}.refit-banner strong,.refit-banner small,.fuel-banner strong,.fuel-banner small{display:block}.refit-banner small,.fuel-banner small{margin-top:.18rem;color:#78a998;font-size:.58rem;line-height:1.4}.fuel-banner small{color:#7199ad}
