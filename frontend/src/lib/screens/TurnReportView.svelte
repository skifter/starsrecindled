<script lang="ts">
  ...
  $: fuelRefills = data?.fuel_refills ?? [];
  $: resourceIncome = data?.resource_income ?? [];
  ...
  $: eventCount = fleetActions.length + refitsStarted.length + refitsCompleted.length + fuelRefills.length + movements.length + colonizations.length + productions.length + installationUpgradesCompleted.length + designsCreated.length + researchCompleted.length + sightings.length;
...
<section class="report-view">
  ...
  {:else}
    {#if eventCount === 0 && warnings.length === 0 && !researchProgress}
      <section class="quiet-turn panel-cut"><Icon name="report" size={22}/><div><strong>Quiet turn</strong><p>No fleet management, refit, refuelling, movement, production, ship design, installation upgrade, colonization, research completion or sensor-contact changes were recorded for your empire.</p></div></section>
    {/if}
    ...
    <section class="report-section panel-cut">
      <div class="section-title"><span><Icon name="energy" size={18}/><strong>Fuel logistics</strong></span><em>{fuelRefills.length}</em></div>
      {#if fuelRefills.length}
        <div class="events">
          {#each fuelRefills as refill}
            <article class="event fuel-event">
              <Icon name="energy" size={18}/>
              <div><strong>{refill.fleetName} refuelled</strong><p>{systemName(refill.systemId)} · +{refill.fuelAdded.toLocaleString('en-US')} fuel · {refill.fuelAfter.toLocaleString('en-US')} / {refill.fuelCapacity.toLocaleString('en-US')}</p></div>
              <button onclick={() => open(refill.systemId)}>Open colony</button>
            </article>
          {/each}
        </div>
      {:else}<p class="none">No fleets required refuelling at your colonies.</p>{/if}
    </section>

    <section class="report-section panel-cut">
      <div class="section-title"><span><Icon name="industry" size={18}/><strong>Resource extraction</strong></span><em>{resourceIncome.length}</em></div>
      {#if resourceIncome.length}
        <div class="events">
          {#each resourceIncome as colony}
            <article class="event economy-event">
              <Icon name="industry" size={18}/>
              <div>
                <strong>{colony.systemName}</strong>
                <p>+{colony.income.industry ?? 0} industry · +{colony.income.energy ?? 0} energy · +{colony.income.bio ?? 0} biomass · +{colony.income.science ?? 0} science{colony.asteroids?.present ? ` · asteroid richness ${colony.asteroids.richness}%` : ''}</p>
              </div>
              <button onclick={() => open(colony.systemId)}>Open colony</button>
            </article>
          {/each}
        </div>
      {:else}<p class="none">No colony resource output was recorded.</p>{/if}
    </section>
      ...
      {#if movements.length}
        <div class="events">
          {#each movements as movement}
            <article class="event">
              <Icon name="fleet" size={18}/>
              <div><strong>{fleetName(movement.fleetId)}</strong><p>{systemName(movement.fromSystemId)} → {systemName(movement.toSystemId)}{typeof movement.distance === 'number' ? ` · ${movement.distance} hop${movement.distance === 1 ? '' : 's'}` : ''}{typeof movement.fuelUsed === 'number' ? ` · −${movement.fuelUsed} fuel · ${movement.fuelRemaining ?? 0}/${movement.fuelCapacity ?? 0} remaining` : ''}</p></div>
