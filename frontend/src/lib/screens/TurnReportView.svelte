<script lang="ts">
  ...
  $: eventCount = movements.length + colonizations.length + productions.length + sightings.length;
...
<section class="report-view">
  ...
  {:else}
    {#if eventCount === 0 && warnings.length === 0}
      <section class="quiet-turn panel-cut"><Icon name="report" size={22}/><div><strong>Quiet turn</strong><p>No movement, production, colonization or sensor-contact changes were recorded for your empire.</p></div></section>
    {/if}
